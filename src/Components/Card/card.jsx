...
import { Items, finalScores } from "../../../fake";
import {
  CartesianGrid,
  Line,
  LineChart,
...
export const Card = () => {
  ...
  const getData = () => {
    ...
    fetch("https://localhost:7090/api/City/GetAllCities", {
      ...
      .then((result) => {
        console.log("Cities fetched:", result);
        setCities(result);
      })
          ...
          <tbody>
            {read.map((item, i) => (
              ...
              >
                ...
                <td className="px-6 py-4 dark:text-white">{item.dcm} µg/m³</td>
                <td className="px-6 py-4 dark:text-white">{item.tempDeg} °C</td>
